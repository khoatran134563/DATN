...
// Tọa độ rơi ống nghiệm (đã cân chỉnh lại độ cao Y = -0.1 để khớp với cốc GLTF mới)
const HOT_DROP = new THREE.Vector3(-1.75, -0.1, 0.1) 
const COLD_DROP = new THREE.Vector3(1.80, -0.1, 0.1) // Xích qua trái một xíu theo ý bạn

const HOME_ROT = -0.06
const CARRIED_ROT = 0.0

// Khôi phục góc nghiêng tuyệt đẹp khi nằm trong cốc
const HOT_DROP_ROT = -0.45 
const COLD_DROP_ROT = 0.45 // Nghiêng đối xứng cho cốc bên phải

// --- XOAY CLAMP ---
const CLAMP_IDLE_ROT = 0
// Xoay -90 độ (Âm PI/2) sẽ hướng phần ngàm kẹp (+Y) sang bên phải (+X)
const CLAMP_ACTIVE_ROT = -Math.PI / 2 
...
export function Scene({ onInfoChange }) {
  ...
  const isClampCarriedRef = useRef(false) 
  const tubeSlotRef = useRef("home")      
  const dragTarget = useRef(new THREE.Vector3())

  const [isClampCarried, setIsClampCarried] = useState(false)
  const [tubeSlot, setTubeSlot] = useState("home")
  ...
  const [mode, setMode] = useState("neutral")
  const [status, setStatus] = useState(
    "Bước 1: Nhấn (Click) vào kẹp gỗ. Kẹp sẽ tự động di chuyển theo chuột."
  )
  ...
  useEffect(() => {
    if (grabTrigger > 0) {
      setStatus("Đã kẹp ống nghiệm! Hãy di chuyển chuột tới gần beaker rồi nhấn Click lần nữa để thả vào.")
    }
  }, [grabTrigger])
  ...
  // ==========================================
  // HÀM TÍNH TOÁN ĐIỂM TIẾP XÚC (ĐÃ SỬA LỖI NGƯỢC ĐẦU)
  // ==========================================
  function getClampTipPoint() {
    if (!clampGroup.current) return new THREE.Vector3()
    // LỖI NẰM Ở ĐÂY CỦA PHIÊN BẢN TRƯỚC:
    // Đã đổi từ -1.0 (Phần tay cầm) sang +1.05 (Phần ngàm kẹp thực tế)
    const localTip = new THREE.Vector3(0, 0.95, 0) 
    localTip.applyEuler(clampGroup.current.rotation)
    return clampGroup.current.position.clone().add(localTip)
  }

  function getTubeNeckPoint() {
    if (!tubeGroup.current) return new THREE.Vector3()
    const localNeck = new THREE.Vector3(0, 1.03, 0) // Cổ ống nghiệm
    localNeck.applyEuler(tubeGroup.current.rotation)
    return tubeGroup.current.position.clone().add(localNeck)
  }

  // --- LOGIC NHẤN/THẢ ---
  const handleDrop = () => {
    if (!isClampCarriedRef.current) return

    isClampCarriedRef.current = false
    setIsClampCarried(false)

    if (tubeSlotRef.current === "carried") {
      const tubePos = tubeGroup.current.position.clone()

      const nearHot = Math.abs(tubePos.x - HOT_POS.x) < 0.8 && tubePos.y < 1.5
      const nearCold = Math.abs(tubePos.x - COLD_POS.x) < 0.8 && tubePos.y < 1.5

      if (nearHot) {
        tubeSlotRef.current = "hot"
        setTubeSlot("hot")
        setMode("hot")
        targetGasRef.current = 1.0
        setStatus("Ống nghiệm đã thả vào nước nóng. Kẹp gỗ tự động thu về giá đỡ.")
      } else if (nearCold) {
        tubeSlotRef.current = "cold"
        setTubeSlot("cold")
        setMode("cold")
        targetGasRef.current = 0.2
        setStatus("Ống nghiệm đã thả vào nước đá. Kẹp gỗ tự động thu về giá đỡ.")
      } else {
        tubeSlotRef.current = "home"
        setTubeSlot("home")
        setMode("neutral")
        targetGasRef.current = 0.68
        setStatus("Đã đặt ống nghiệm về giá đỡ.")
      }
    } else {
      setStatus("Đã đặt kẹp xuống. Nhấn vào kẹp để cầm lên lại.")
    }
  }
  ...
  useFrame((state) => {
    ...
    // 1. Khí & Ánh sáng
    gasIntensityRef.current = THREE.MathUtils.lerp(gasIntensityRef.current, targetGasRef.current, 1 - Math.exp(-1.6 * dt))
    if (Math.abs(gasIntensityRef.current - gasIntensity) > 0.002) setGasIntensity(gasIntensityRef.current)

    if (ambientRef.current) ambientRef.current.intensity = 0.34 + Math.sin(t * 0.35) * 0.008
    if (keyLightRef.current) keyLightRef.current.intensity = 2.2 + Math.sin(t * 0.45) * 0.04
    if (hotModeLightRef.current) hotModeLightRef.current.intensity = (mode === "hot" ? 0.45 : 0.06) + Math.sin(t * 2.3) * 0.025
    if (coldModeLightRef.current) coldModeLightRef.current.intensity = (mode === "cold" ? 0.42 : 0.06) + Math.sin(t * 1.9 + 0.6) * 0.02

    // 2. Clamp
    const clampTargetRot = isClampCarriedRef.current ? CLAMP_ACTIVE_ROT : CLAMP_IDLE_ROT
    clampGroup.current.rotation.z = THREE.MathUtils.lerp(clampGroup.current.rotation.z, clampTargetRot, 0.16)

    if (isClampCarriedRef.current) {
      const x = THREE.MathUtils.clamp(state.pointer.x * 3.6, -3.2, 3.2)
      const y = THREE.MathUtils.clamp(0.9 + state.pointer.y * 2.25, -0.2, 2.5)
      dragTarget.current.set(x, y, 0.12)
      clampGroup.current.position.lerp(dragTarget.current, 0.2)

      if (tubeSlotRef.current !== "carried") {
        const clampTip = getClampTipPoint()
        const tubeNeck = getTubeNeckPoint()
        
        if (clampTip.distanceTo(tubeNeck) < 0.45) {
          tubeSlotRef.current = "carried"
          setTubeSlot("carried")
          setGrabTrigger(prev => prev + 1) 
        }
      }
    } else {
      const clampIdleBob = tubeSlotRef.current !== "carried" ? Math.sin(t * 1.3) * 0.012 : 0
      clampGroup.current.position.lerp(new THREE.Vector3(CLAMP_HOME.x, CLAMP_HOME.y + clampIdleBob, CLAMP_HOME.z), 0.1)
    }

    // 3. TestTube
    if (tubeSlotRef.current === "carried") {
      const clampTip = getClampTipPoint()
      const neckOffset = new THREE.Vector3(0, 1.03, 0).applyEuler(tubeGroup.current.rotation)
      const carryWobble = Math.sin(t * 10.5) * 0.005
      
      const targetTubePos = clampTip.clone().sub(neckOffset).add(new THREE.Vector3(-0.02, 0, 0))
      
      tubeGroup.current.position.lerp(targetTubePos, 0.3)
      tubeGroup.current.rotation.z = THREE.MathUtils.lerp(tubeGroup.current.rotation.z, CARRIED_ROT + carryWobble, 0.15)
    } else {
      let targetPos = TUBE_HOME.clone()
      let targetRot = HOME_ROT

      if (tubeSlotRef.current === "home") {
        targetPos.y += Math.sin(t * 1.2 + 0.7) * 0.012
        targetRot += Math.sin(t * 1.3) * 0.005
      } else if (tubeSlotRef.current === "hot") {
        // Cập nhật lại logic nằm nghiêng (có hiệu ứng bồng bềnh của nước)
        targetPos = HOT_DROP.clone().add(new THREE.Vector3(0, Math.sin(t * 1.5) * 0.005, 0))
        targetRot = HOT_DROP_ROT + Math.sin(t * 2.8) * 0.003
      } else if (tubeSlotRef.current === "cold") {
        // Cập nhật lại logic nằm nghiêng
        targetPos = COLD_DROP.clone().add(new THREE.Vector3(0, Math.sin(t * 1.2) * 0.004, 0))
        targetRot = COLD_DROP_ROT + Math.sin(t * 2.5 + 0.4) * 0.003
      }

      tubeGroup.current.position.lerp(targetPos, 0.1)
      tubeGroup.current.rotation.z = THREE.MathUtils.lerp(tubeGroup.current.rotation.z, targetRot, 0.1)
    }
  })
  ...
  return (
    <>
      ...
      <ambientLight ref={ambientRef} intensity={0.34} />
      <directionalLight ref={keyLightRef} position={[-2.8, 5.8, 3.6]} intensity={2.2} castShadow />
      <pointLight ref={hotModeLightRef} position={[-1.9, 0.4, 0.7]} color="#ff9f52" />
      <pointLight ref={coldModeLightRef} position={[1.9, 0.4, 0.7]} color="#9fdcff" />
      <Environment preset="night" />
      <ContactShadows position={[0, -1.26, 0]} opacity={0.55} scale={8} blur={2.6} />
      ...
      <group ref={tubeGroup} scale={0.92}>
        <TestTube gasIntensity={gasIntensity} active={tubeSlotRef.current === "carried"} />
      </group>
      ...
      <group
        ...
        onClick={(e) => {
          e.stopPropagation()
          if (!isClampCarriedRef.current) {
            isClampCarriedRef.current = true
            setIsClampCarried(true)
            setStatus("Đang cầm kẹp. Hãy di chuyển ngàm kẹp tới gần cổ ống nghiệm để tự động gắp.")
          } else {
            handleDrop()
          }
        }}
      ...
      >
        {/* Đã xóa <group rotation> hack lật 180 độ ở đây, Clamp sẽ hiển thị tự nhiên! */}
        <Clamp active={isClampCarried || tubeSlot === "carried"} />
      </group>
      ...
      <OrbitControls enablePan={false} enableRotate={false} minDistance={7} maxDistance={10} target={[0, 0.04, 0]} />
    </>
  )
}