...
const QuestionModal = ({
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-scale-up border border-gray-100"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h3 className="text-xl font-bold text-gray-800">
            {isEditing ? 'Cập nhật câu hỏi' : 'Thêm câu hỏi mới'}
          </h3>
        ...
        <div className="p-6 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50">
          <button
            onClick={onClose}
            disabled={saving}
            className="px-5 py-2.5 font-bold text-gray-600 bg-white border border-gray-300 hover:bg-gray-50 rounded-xl transition-all disabled:opacity-50"
          >
            Hủy bỏ
          </button>
          ...
          <button
            onClick={onSave}
            disabled={saving}
            className="px-6 py-2.5 font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg shadow-blue-500/30 transition-all hover:scale-105 disabled:opacity-60 disabled:hover:scale-100"
          >
            {saving ? 'Đang lưu...' : isEditing ? 'Cập nhật' : 'Lưu câu hỏi'}
          </button>
        </div>
      </div>
    </div>
