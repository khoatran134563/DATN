...
const Header = () => {
  ...
  const isTeacher = userRole === 'teacher' || userRole === 'gv';
    ...
    >
      <div className="flex items-center">
        <Link
          ...
          className="flex items-center gap-2 group relative mr-4"
        >
          ...
          <div className="bg-gradient-to-br from-yellow-300 to-yellow-500 p-1.5 rounded-xl text-blue-900 group-hover:rotate-12 transition-transform duration-300 shadow-[0_4px_15px_rgba(250,204,21,0.4)] relative z-10 border border-yellow-200">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 drop-shadow-sm" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M7 2a1 1 0 00-.707 1.707L7 4.414v3.758a1 1 0 01-.293.707l-4 4C.817 14.769 2.156 18 4.828 18h10.344c2.673 0 4.012-3.231 2.122-5.121l-4-4A1 1 0 0113 8.172V4.414l.707-.707A1 1 0 00113 2H7zm2 6.172V4h2v4.172a3 3 0 00.879 2.12l1.027 1.028a4 4 0 00-2.171.102l-.47.156a4 4 0 01-2.53 0l-.563-.187a1.993 1.993 0 00-.114-.035l1.063-1.063A3 3 0 009 8.172z" clipRule="evenodd" />
            </svg>
          </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative">
          <button
            ...
            className="focus:outline-none flex items-center gap-3 group p-1 rounded-full transition-all duration-300 hover:bg-white/10"
          >
            {isLoggedIn && (
            ...
            <div
              className={`relative w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shadow-lg
                ${isLoggedIn
                  ? (userRole === 'admin'
                      ? 'bg-gradient-to-br from-purple-500 to-indigo-600 p-[2px]'
                      : 'bg-gradient-to-br from-yellow-300 via-orange-400 to-red-500 p-[2px]')
            ...
            >
              ...
              >
                {isLoggedIn ? (
                  <span
                    className={`font-black text-sm w-full h-full flex items-center justify-center
                      ${userRole === 'admin'
                        ? 'text-purple-700 bg-purple-50'
                        : 'text-blue-900 bg-gradient-to-br from-gray-100 to-gray-300'}`}
                  >
                    {userRole === 'admin' ? 'AD' : (isTeacher ? 'GV' : 'HS')}
                  </span>
                ) : (
                  ...
                  {isLoggedIn ? (
                    <>
                      <div className="px-6 py-5 bg-gradient-to-r from-blue-600/10 to-indigo-600/10 border-b border-blue-100/50 relative">
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-14 h-14 rounded-full p-[3px] shadow-md
                              ${userRole === 'admin'
                          ...
                          >
                            <div className="w-full h-full rounded-full bg-white flex items-center justify-center">
                              <span className={`font-black text-xl ${userRole === 'admin' ? 'text-purple-700' : 'text-blue-900'}`}>
                                {userRole === 'admin' ? 'A' : (isTeacher ? 'G' : 'H')}
                              </span>
                            </div>
                          </div>
                      ...
                      <div className="p-3 space-y-1">
                        <Link
                          to="/profile"
                          className="flex items-center px-4 py-3 text-sm font-bold text-gray-700 rounded-2xl hover:bg-blue-500/10 hover:text-blue-700 transition-all group"
                          onClick={() => setIsUserMenuOpen(false)}
                        >
                        ...
                        {isTeacher && (
                          <Link
                            to="/my-posts"
                            className="flex items-center px-4 py-3 text-sm font-bold text-gray-700 rounded-2xl hover:bg-emerald-500/10 hover:text-emerald-700 transition-all group"
                            onClick={() => setIsUserMenuOpen(false)}
                          >
                            <span className="mr-4 text-xl bg-emerald-100/60 p-2 rounded-xl group-hover:bg-emerald-200/60 group-hover:scale-110 transition-all shadow-sm">📝</span>
                            <span className="flex-1">Trạng thái bài đăng</span>
                          </Link>
                        )}
                      </div>
                      ...
                      <div className="p-3 mt-1">
                        <button
                          ...
                          className="flex w-full items-center justify-center gap-2 px-4 py-3.5 text-sm font-bold text-red-600 bg-red-50/50 hover:bg-red-100/80 rounded-2xl transition-all shadow-sm hover:shadow-md active:scale-[0.98]"
                        >
                    ...
                    <div className="p-6 relative">
                      <div className="text-center mb-6 relative z-10">
                        ...
                        <p className="text-sm text-gray-500 font-medium">
                          Hãy bắt đầu hành trình khám phá hóa học.
                        </p>
                      </div>
                      ...
                      <div className="space-y-4 relative z-10">
                        <Link
                          to="/signin"
                          className="group relative w-full flex items-center justify-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold text-white overflow-hidden transition-all duration-300 shadow-lg bg-blue-600 hover:bg-blue-700"
                          onClick={() => setIsUserMenuOpen(false)}
                        >
                        ...
                        <Link
                          to="/signup"
                          className="group relative w-full flex items-center justify-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold text-white overflow-hidden transition-all duration-300 shadow-lg bg-emerald-500 hover:bg-emerald-600"
                          onClick={() => setIsUserMenuOpen(false)}
                        >
